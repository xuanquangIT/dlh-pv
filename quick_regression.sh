...
echo "----------------------------------------"
docker cp "$SCRIPT_DIR/src/pv_lakehouse/mlflow/train_regression_model.py" spark-master:/opt/spark/work-dir/train_regression_model.py
docker compose --env-file ../.env exec -T spark-master spark-submit --master spark://spark-master:7077 --driver-memory 2g --executor-memory 4g train_regression_model.py --limit 0 
...
docker compose --env-file ../.env exec -T trino trino \
