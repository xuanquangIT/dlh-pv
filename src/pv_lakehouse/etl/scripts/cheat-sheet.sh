...
echo "🏆 GOLD LAYER - Analytics & Data Mart (Star Schema)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "# Dimension Tables (Load in order - these first)"
echo "docker compose -f docker/docker-compose.yml exec spark-master bash -c \"cd /opt/workdir && python3 -m pv_lakehouse.etl.gold.cli dim_facility --mode full\""
echo "docker compose -f docker/docker-compose.yml exec spark-master bash -c \"cd /opt/workdir && python3 -m pv_lakehouse.etl.gold.cli dim_date --mode full\""
echo "docker compose -f docker/docker-compose.yml exec spark-master bash -c \"cd /opt/workdir && python3 -m pv_lakehouse.etl.gold.cli dim_time --mode full\""
echo "docker compose -f docker/docker-compose.yml exec spark-master bash -c \"cd /opt/workdir && python3 -m pv_lakehouse.etl.gold.cli dim_aqi_category --mode full\""
echo ""
echo "# Fact Table (Load after dimensions)"
echo "docker compose -f docker/docker-compose.yml exec spark-master bash -c \"cd /opt/workdir && python3 -m pv_lakehouse.etl.gold.cli fact_solar_environmental --mode full\""
echo ""
echo "# Incremental Load (For daily/hourly updates after initial load)"
echo "docker compose -f docker/docker-compose.yml exec spark-master bash -c \"cd /opt/workdir && python3 -m pv_lakehouse.etl.gold.cli fact_solar_environmental --mode incremental\""
echo ""
...
echo "docker compose -f docker/docker-compose.yml exec trino trino --execute \\"
echo "  \"SELECT 'dim_facility' as table_name, COUNT(*) as row_count FROM iceberg.gold.dim_facility \\"
echo "   UNION ALL SELECT 'dim_date', COUNT(*) FROM iceberg.gold.dim_date \\"
echo "   UNION ALL SELECT 'dim_time', COUNT(*) FROM iceberg.gold.dim_time \\"
echo "   UNION ALL SELECT 'dim_aqi_category', COUNT(*) FROM iceberg.gold.dim_aqi_category;\""
echo ""
echo "# Gold - Fact table summary"
echo "docker compose -f docker/docker-compose.yml exec trino trino --execute \\"
echo "  \"SELECT COUNT(*) as total_records, \\"
echo "   COUNT(DISTINCT facility_key) as facilities, \\"
echo "   COUNT(DISTINCT date_key) as dates, \\"
echo "   COUNT(DISTINCT time_key) as times, \\"
echo "   ROUND(AVG(completeness_pct), 2) as avg_completeness \\"
echo "   FROM iceberg.gold.fact_solar_environmental;\""
echo ""
echo "# Gold - Check for duplicates (should be 1.0 for all)"
echo "docker compose -f docker/docker-compose.yml exec trino trino --execute \\"
echo "  \"SELECT COUNT(*) as total_rows, \\"
echo "   MAX(cnt) as max_duplicates, \\"
echo "   AVG(CAST(cnt AS DOUBLE)) as avg_duplicates \\"
echo "   FROM (SELECT facility_key, date_key, time_key, COUNT(*) as cnt \\"
echo "         FROM iceberg.gold.fact_solar_environmental \\"
echo "         GROUP BY facility_key, date_key, time_key);\""
echo ""
echo "# Gold - Sample data"
echo "docker compose -f docker/docker-compose.yml exec trino trino --execute \\"
echo "  \"SELECT facility_key, date_key, time_key, energy_mwh, \\"
echo "   shortwave_radiation, temperature_2m, pm2_5, completeness_pct, is_valid \\"
echo "   FROM iceberg.gold.fact_solar_environmental LIMIT 10;\""
echo ""

echo "🤖 MLFLOW - Machine Learning"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo "  • ✅ All Silver tables: Grain = (facility_code, date_hour)"
echo "  • ✅ All Gold hourly facts: Grain = (facility_key, date_key, time_key)"
echo "  • ✅ Gold daily facts: Grain = (facility_key, date_key)"
echo "  • ✅ Duplicate checks: max_dup=1.0, avg_dup=1.0 for all tables"
echo "  • Bug fixed: Removed daily aggregation in weather/air_quality lookups"
echo "  • Bug fixed: Added specific issue_type join in fact_root_cause_analysis"
echo ""
...
echo "  • ✅ Silver chunking: 3-day (energy), 7-day (weather/air_quality)"
